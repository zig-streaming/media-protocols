            ...
            if (c) |candidate| switch (candidate) {
                .host => |socket| {
                    ...
                    if (maybe_host_candidate) |host_candidate| {
                        ...
                        try agent.on_event(agent.userdata, agent, .{ .candidate = host_candidate });
                ...
                .srflx => |srflx| {
                    ...
                    try agent.sockets.append(agent.core.allocator, socket);
                    try agent.group.concurrent(io, receive, .{ agent, socket });
                    try agent.on_event(agent.userdata, agent, .{ .candidate = added.? });
                },
                .relay => |client| {
                    errdefer {
                        client.deinit(io);
                    ...
                    if (added) {
                        ...
                        try agent.on_event(agent.userdata, agent, .{ .candidate = relay_c });
                    } else {
                        client.deinit(io);
                        agent.core.allocator.destroy(client);
                    }
